...
const getGame = (id) =>
  db('game')
    ...
    .where({ 'game.id': id })
    .joinRaw(
      `LEFT JOIN (SELECT ga.game_id, array_agg(to_json(ga)) AS azs FROM azs ga GROUP BY ga.game_id) a ON a.game_id = game.id`,
    )
    .joinRaw(
      `LEFT JOIN (SELECT gs.game_id, array_agg(to_json(gs)) AS staff FROM staff gs GROUP BY gs.game_id) s ON s.game_id = game.id`,
    )
    .joinRaw(
      `LEFT JOIN (SELECT gt.game_id, array_agg(to_json(gt)) AS tunker FROM tunker gt GROUP BY gt.game_id) t ON t.game_id = game.id`,
    )
      .joinRaw(
          `LEFT JOIN (SELECT wp.azs_id, array_agg(to_json(wp)) AS place FROM work_place wp GROUP BY wp.azs_id) p ON p.azs_id = a.id`,
      )
    .joinRaw(
      `LEFT JOIN (SELECT gw.game_id, array_agg(to_json(gl)) AS logs FROM game_log gl GROUP BY gl.game_id) l ON l.game_id = game.id`,
    )
    .first();
